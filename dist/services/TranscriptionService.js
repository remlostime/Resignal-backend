...
export class TranscriptionService {
    ...
    async uploadChunk(jobId, userId, chunkIndex, audioBuffer, filename, mimetype) {
    ...
    validateAudioFile(filename, mimetype, size) {
        if (!ALLOWED_AUDIO_MIME_TYPES.includes(mimetype)) {
            throw new ServiceError(`Invalid audio type "${mimetype}". Allowed: ${ALLOWED_AUDIO_MIME_TYPES.join(", ")}`, 400);
        }
        const ext = filename.slice(filename.lastIndexOf(".")).toLowerCase();
        if (!ALLOWED_EXTENSIONS.includes(ext)) {
            throw new ServiceError(`Invalid file extension "${ext}". Allowed: ${ALLOWED_EXTENSIONS.join(", ")}`, 400);
        }
        if (size > MAX_CHUNK_SIZE_BYTES) {
            throw new ServiceError(`File size ${(size / 1024 / 1024).toFixed(1)}MB exceeds the 25MB limit`, 400);
        }
    }
}
